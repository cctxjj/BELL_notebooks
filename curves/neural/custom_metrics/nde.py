...
def predict_drag(model, points):
    points_formated = np.array(converge_shape_to_mirrored_airfoil(points))
...
def compare_predictions(n):
    model = tf.keras.models.load_model("C:\\Users\\Sebastian\\PycharmProjects\BELL_notebooks/data/models/cd_prediction_model_1.keras")
    for i in range(n):
        cont_points = create_random_curve_points(5, random.randint(0, 3), random.randint(6, 13), 0,
                                                 random.randint(1, 15))
        points = bezier_curve(cont_points, 100)
